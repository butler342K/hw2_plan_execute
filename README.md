...
| Крок | Вузол | Що відбулось | Час від старту |
|------|-------|--------------|-----------------|
| 1 | planner | Побудував план з 3 кроків: перевірити замовлення → відстежити посилку → порівняти дані | 1363 мс |
| 2 | executor | Крок 1: `search_order` знайшов замовлення, статус «доставлено», ТТН 20400500577861 | 3045 мс |
| 3 | replanner | `action=continue` — план актуальний, іти далі | 3719 мс |
| 4 | executor | Крок 2: `track_parcel` за знайденим ТТН → Нова Пошта: «Номер не знайдено» | 6152 мс |
| 5 | replanner | `action=finish` — даних достатньо, розбіжність зафіксована в response | 7759 мс |
| 6 | respond | Структурована відповідь (`resolved: true`) | 9162 мс |
